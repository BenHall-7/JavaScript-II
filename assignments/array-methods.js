...
let firstNamesAllCaps = runners.map(person => person.first_name.toUpperCase());
...
// Problem 1
// The person with the lowest donation is sacrificed in a volcano. Who is removed, and how many runners are left?
console.log(
  runners.splice(
    runners.reduce(
      (leastIndex, person, index, arr) => {
        return person.donation < arr[leastIndex].donation ? index : leastIndex;
      },
      0
    ),
    1
...
console.log(`runners left: ${runners.length}`);
